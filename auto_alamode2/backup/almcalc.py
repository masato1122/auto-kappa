...
2. Error in mp-252
    Check auto_alamode !!
    Error with ALM.
    dfset is correct, but FCs are not calculated properly.
...
class AlmCalc():
    ...
    def __init__(self, str_given, mpid='mp', 
        ...
        ### structures
        self._primitive = change_structure_format(str_given, 'ase')
    ...
    def get_suggested_structures(self, order=None):
        """ Get structures suggested by ALM """
    ...
    def calc_forces(self, order=1, 
            nprocs=1, calculator=None,
        ...
        if order == 1:
            ...
            print(" Error: order=%d is not supported." % (order))
            exit()
        
        structures = self.get_suggested_structures(order=order)
        
        calculator.command = "%s -n %d %s" % (mpirun, nprocs, vasp)
         
        print("")
        print(" ### Calculate forces...")
        print(" order: %d" % (order))
        print(" Number of structures: %d" % (len(structures)))
        print("")
    ...
    def calc_harmonic_force_constants(self, output=True, delete=True):
        ...
        if os.path.exists(out_dfset) == False:
            ...
            data = np.loadtxt(out_dfset).reshape((-1, natoms, 6))[[0]]
        ...
        alm = _get_alm(structure)

        ## calculate force constants
        alm = calc_force_constants(alm,
                displacements=disps, forces=forces,
                order=1, cutoff=self.cutoffs[0], nbody=nbody,
                outfile=out_xml)
    ...
    def calc_cubic_force_constants(self):
        ...
        ## set ALM
        alm = self.calc_harmonic_force_constants(output=False, delete=False)
        
        ## harmonic FCs
        fc2_values, elem2_indices = alm.get_fc(1)
        
        ## 3rd order model
        alm.define(2, cutoff_radii=self.cutoffs[:2], nbody=nbody,
                symmetrization_basis="Cartesian")
        alm.displacements = disps
        alm.forces = forces
        
        ## fix harmonic FCs
        alm.freeze_fc(fc2_values, elem2_indices)
        info = alm.optimize()
        alm.save_fc(out_xml, format='alamode')
        print(" Output", out_xml) 
    ...
    def run_anphon(self, propt='band', nprocs=1, nthreads=None, force=False):
        ...
        ##
        print("")
        print(" ### Anphon calculation (%s) " % propt)
...
def _get_alm_structure(str_given):
    """ get ALM object
    Note : length unit is Bohr
    """
    if type(str_given) == "ase.atoms.Atoms":
        atoms = str_given.copy()
    else:
        atoms = change_structure_format(str_given, format='ase')
    
    lave = atoms.cell * AToBohr
    xcoord = atoms.get_scaled_positions()
    kd = atoms.get_atomic_numbers()
    
    return lave, xcoord, kd

    #alm = ALM(lave, xcoord, kd)
    #alm.alm_new()
    #alm.set_verbosity(0)
    #return alm

def get_displacements(str_given, order, cutoffs=None, nbody=None, mag=None):
    ...
    mag : unit=[Ang]
    """
    lave, xcoord, kd = _get_alm_structure(str_given)
    
    natoms = len(alm.numbers)
    ...
    with ALM(lave, xcoord, kd) as alm:
        alm.set_verbosity(0)
        alm.define(order, cutoff_radii=cutoffs, nbody=nbody_new,
                symmetrization_basis="Cartesian")
        info = alm.suggest()
