...
class AlamodeForceCalculator():
    ...
    def _job_for_each_structure(
        self, job_idx, structures, base_dir, order, calculator, use_mlips=False, **amin_params_set):
        ...
        if job_idx == 0:
            if hasattr(calculator, 'asdict') and not use_mlips:
                print_vasp_params(calculator.asdict()['inputs'])
            elif use_mlips:
                logger.info(f"\n Using MLIPs calculator: {type(calculator).__name__}")
            logger.info("")
        ...
        if self.calculate_forces:
            ...
            logger.info(f"\n Calculate_forces is False, skipping force calculation")
            
        ### print log
        time_min = self._print_each_end(
            job_idx + 1, len(struct_keys),
            self.get_relative_path(calculator.directory if hasattr(calculator, 'directory') else outdir))
    ...
    def _calculate_forces_mlips(self, calculator, structure, outdir):
        ...
        try:
            ...
            #if 'prist' in outdir: # Force is zero for pristine structure
            #    forces = np.zeros_like(forces, dtype=np.float64)
