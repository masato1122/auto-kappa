...
def _rank_error(logfile, tar=None):
    lines = file2str(logfile, tar=tar)
    
    loc = None
    flag = False
    for line in lines:
        if "Error found in fc2.log" in line:
            loc = 'fc2'
        if "Error found in fc3.log" in line:
            loc = 'fc3'
        if 'Error: Rank deficient' in line:
            flag = True
            break
    # return [flag, loc]
    return flag
...
def _include_calculation_times(logfile, tar=None):
    
    lines = file2str(logfile, tar=tar)
    
    flag = False
    for ll in lines:
        if "Calculation times:" in ll:
            return True
...
def finish_ak_calculation(dir_tmp, neg_freq=-0.001, vol_relax=None, larger_sc=None):
    """ Check the result of the auto-kappa calculation. """
    ...
    if _exists(logfile, tar=tar) == False:
        ...
        elif _rank_error(logfile, tar=tar):
            return [True, "rank_error"]
    ...
    if status_harm[0].lower() == "finished":    
        ...
        if fmin < neg_freq:
            ...
            ### check log file
            if _exists(logfile, tar=tar):
                calc_time = _include_calculation_times(logfile, tar=tar)
